...
## Generated list screens

M3.12 materializes validated `ScreenDefinition(kind: .list)` entries as normal Flutter source on top of the M3.11 `DomainRecord<T>` read contract.

The deterministic pipeline is:

```text
ScreenDefinition.list
  -> entityID
  -> ordered visibleFieldIDs
  -> GeneratedListFieldValue entries
  -> Future<List<DomainRecord<Entity>>> loader
  -> Loading / Empty / Error / Data UI
```

A generated list screen never instantiates a repository, SQLite data source, Supabase client, Firebase client, or AppForge runtime. The caller injects a `GeneratedListLoader<T>`; an optional selection callback receives the complete `DomainRecord<T>` so record identity is not lost between list and later detail/edit flows.

`visibleFieldIDs` is authoritative for displayed business fields and their order. The technical `recordId` is not rendered as a business field. It is used as the stable Flutter `ValueKey<String>` for each record row and is passed only through the identified record contract.

The reusable list runtime explicitly materializes:

- loading state with a progress indicator;
- empty state with a caller-overridable message;
- error state with a caller-overridable safe message and retry action;
- data state with deterministic record rows;
- optional record selection without dropping `recordId`.

Display formatting remains provider- and locale-independent in this slice:

- strings, enums and numeric values use their stable textual representation;
- booleans render as `Yes` / `No`;
- date/time values use UTC ISO-8601-derived text;
- file/image values render their URI;
- colors render their normalized hex value;
- locations render latitude and longitude.

List screen output paths and generated widget types participate in the same pre-render collision policy as form screens and entity-derived types. List screens without an entity fail closed before source emission.

M3.12 deliberately does not add repository composition, navigation, detail/edit/delete actions, pagination, relation display, search/filter/sort, or remote querying. Those capabilities build on the identified list boundary rather than bypassing it.

## Generated detail screens

M3.13 materializes validated `ScreenDefinition(kind: .detail)` entries as read-only Flutter detail screens built on the stable `DomainRecord` identity contract.

The deterministic pipeline is:

```text
ScreenDefinition.detail
  -> entityID
  -> ordered visibleFieldIDs
  -> DomainRecord<Entity>
  -> shared GeneratedRecordFieldValue entries
  -> read-only Material detail UI
```

The technical record ID stays available for later edit/delete/navigation flows but is not rendered as a business field. The generated detail runtime uses the record ID only as stable widget identity and passes the complete identified record to an optional edit-intent callback.

List and detail screens share one generated `generated_record_display.dart` contract. It owns value-kind metadata and deterministic display conversion for scalar, date/time and rich domain values. This prevents list/detail drift in how file/image URIs, normalized colors, locations and UTC ISO-8601 date/time values are displayed.

A detail wrapper receives its `DomainRecord<Entity>` from the caller. It does not load a repository, invoke SQLite, create a provider client, save changes or perform navigation. An optional edit callback expresses intent only and receives the full identified record.

Detail screen output paths and widget types participate in the same fail-closed collision validation as entity, form and list output. A detail screen without a valid entity fails before source emission.

M3.13 deliberately leaves edit-form prefill, save/delete execution, navigation/routes, relation display and provider composition to later slices.

## Record-aware form bridge

M3.14 connects identified read records to generated form wrappers without moving persistence or ID policy into the UI layer.

An entity-specific form wrapper can now receive an optional `DomainRecord<Entity>`. When present, the wrapper projects exactly the screen's ordered `visibleFieldIDs` from `record.value` into typed form initial values. Hidden entity fields are not injected into the edit form.

The bridge preserves the existing generic form runtime. `GeneratedEntityFormScreen` still receives only field specs, initial values and a values-only submit callback. It remains responsible for validation, normalization and creation of an immutable values snapshot.

The entity-specific wrapper adapts that callback to `GeneratedIdentifiedFormSubmit`:

```text
DomainRecord<Entity>? record
  -> visible-field typed prefill
  -> GeneratedEntityFormScreen
  -> validated + normalized immutable values
...
This gives edit flows the existing technical ID while create flows deliberately submit `recordId == null`. M3.14 does not invent UUIDs or provider IDs.

Explicit caller-provided `initialValues` are layered after record-derived values, allowing intentional per-field overrides without changing the record itself. Final form submission still includes only fields defined by the generated form specs.

Rich domain values remain typed during prefill: file, image, color and location values are passed directly from the entity rather than serialized to strings.

This slice does not instantiate repositories or use cases, execute save/delete operations, perform navigation, or introduce provider clients. Those remain composition/runtime concerns for later slices.

## Typed edit mapper

M3.15 closes the deterministic edit-value round trip without moving persistence into generated presentation code.

Every entity-specific form wrapper exposes a static typed edit mapper. It receives the existing `DomainRecord<Entity>` together with the normalized immutable values snapshot emitted by the form runtime and returns a new complete entity instance.

```text
DomainRecord<Entity> record
        +
normalized Map<String, Object?> values
        ↓
generated typed edit mapper
        ↓
new Entity(
  visible fields <- normalized values,
  hidden fields  <- record.value,
  source relations <- record.value
)
```

Exactly the form screen's `visibleFieldIDs` are replaced. Hidden fields are copied from the existing entity value, and all source-side relations remain unchanged. The existing record is never mutated.

Visible values fail closed. A visible key must be present even for optional fields; optional fields may contain `null`, while required fields may not. Non-null values must match the generated Dart domain type. The mapper does not use heuristic defaults or silently retain an old visible value when the normalized snapshot is incomplete.

String, numeric, boolean, date/time and rich file/image/color/location values remain typed throughout the mapping step. Relations are not editable in this slice and are therefore copied from `record.value`.

The mapper only reconstructs a typed entity. It does not generate record IDs, instantiate repositories or save use cases, perform save/delete execution, navigate, or create provider clients. Create mapping remains a separate later contract because there is no existing entity from which hidden required fields can be preserved safely.
...
M3.15 owns deterministic domain contracts, stable record identity across generated read paths, offline mappings, generated-contract collision validation, field-based Flutter form screens, identified Flutter list/detail screens, shared record display formatting, a record-aware form bridge, and typed edit reconstruction that preserves hidden fields and source-side relations. Navigation/route materialization, relation placement in screens, save/delete execution, dashboard/settings/custom screens, create-ID policy, repository composition and provider backend rendering remain later slices.
