...
M3.13 owns deterministic domain contracts, stable record identity across generated read paths, offline mappings, generated-contract collision validation, field-based Flutter form screens, identified Flutter list screens, shared record display formatting, and identified read-only Flutter detail screens. Navigation/route materialization, relation placement in screens, edit/delete wiring, dashboard/settings/custom screens, create-ID policy, repository composition/submit wiring, and provider backend rendering remain later slices.
