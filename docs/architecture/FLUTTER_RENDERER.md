...
M3.11 binds that callback in the generated application composition root when offline storage is enabled. The generated runtime now follows this concrete path:

```text
GeneratedAppHome
  -> generated form screen
  -> screen-specific form ViewModel
  -> Save<Entity> use case
  -> <Entity>Repository
  -> SQLite local data source
  -> local SSOT + atomic sync outbox
```

The start screen lists configured form destinations in navigation order and falls back to deterministic screen order for forms without a navigation item. A successful save returns to the start screen and emits accessible confirmation. Back navigation remains native Flutter navigation.

New records receive RFC 4122 version 4 identifiers from `Random.secure()`. IDs are generated behind a `RecordIdGenerator` boundary so business logic remains testable and does not use timestamps as identifiers.

Field defaults are injected into the rendered form and hidden required fields may only be constructed when a validated default exists. The renderer fails closed when a generated create flow would otherwise require:

- a missing required field;
- a required relation picker;
- a required external file, image, color, or location picker;
- role evaluation that has not yet been materialized; or
- a repository while offline persistence is disabled.

This prevents generated screens from exposing unauthorized actions or pretending to save data. Optional external fields retain the provider-neutral picker boundary and may be left empty until a concrete adapter is selected.
...
M3.11 owns deterministic domain contracts, offline mappings, generated-contract collision validation, field-based Flutter forms, create-route materialization, and local repository submit wiring. Relation placement and pickers, authenticated role evaluation, list/detail/dashboard/settings/custom screens, remote synchronization workers, and provider backend rendering remain later slices.
