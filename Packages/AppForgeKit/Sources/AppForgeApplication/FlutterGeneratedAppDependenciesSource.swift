...
struct FlutterGeneratedAppDependenciesSource {
    let routes: [FlutterGeneratedFormRoute]
...
private extension FlutterGeneratedAppDependenciesSource {
    func content() throws -> String {
        ...
        lines += ["  });", "", "  factory AppDependencies.production() {"]
        lines += try productionFactoryLines(entities: entities)
        lines += ["  }", "", "  final AppDatabase database;"]
        lines += try routes.map { route in
            let typeName = try FlutterFormRenderingSupport.viewModelTypeName(
                for: route.screen
            )
    ...
    func importLines(
        ...
        for route in routes {
            let feature = FlutterDartNaming.snakeCase(route.entity.identity.code)
            let screen = FlutterDartNaming.snakeCase(route.screen.identity.code)
            imports.append(
                "import '../../features/\(feature)/presentation/view_models/\(screen)_form_view_model.dart';"
            )
        }
    ...
    func productionFactoryLines(
        ...
        lines += ["    return AppDependencies._(", "      database: database,"]
        for route in routes {
            let entityType = FlutterDartNaming.typeName(route.entity.identity.code)
            let repository = FlutterDartNaming.memberName(route.entity.identity.code)
            let viewModel = try FlutterFormRenderingSupport.viewModelTypeName(
                for: route.screen
            )
            lines += [
